...
function pb_outlook_exchange_code_for_tokens($code) {
    ...
    // Registrar evento
    pb_log_security_event(get_current_user_id(), 'oauth_token_exchange', [
        'success' => false,
        'error' => null
    ]);
...
' . $clean_body . '
    ...
    $response = wp_remote_post($sendMailUrl, array(
        'headers' => array(
    ...
    if ($status_code >= 200 && $status_code < 300) {
        // Registrar evento
        pb_log_security_event($user_id, 'email_sent', ['to' => $to, 'subject' => $subject]);
        return true;
    }
    // Registrar el error para análisis
    error_log('Error al enviar correo. Respuesta HTTP: ' . $status_code . ' => ' . wp_remote_retrieve_body($response));
    return new WP_Error('send_mail_error',
...
function pb_outlook_disconnect_user($user_id) {
    try {
        ...
        if (!get_user_by('id', $user_id)) {
            ...
            pb_log_security_event($user_id, 'outlook_disconnect_failed', [
        ...
        pb_log_security_event($user_id, 'outlook_disconnect_completed', [
            'success' => $success,
            'results' => $results
        ]);
        ...
        pb_log_security_event($user_id, 'outlook_disconnect_error', [
...
function pb_outlook_disconnect_ajax_handler() {
    ...
    try {
        ...
        if (!isset($_POST['nonce'])) {
            pb_log_security_event(0, 'outlook_disconnect_ajax_failed', ['reason' => 'missing_nonce']);
        ...
        if (!$nonce_verification) {
            pb_log_security_event(0, 'outlook_disconnect_ajax_failed', ['reason' => 'invalid_nonce']);
        ...
        if (!is_user_logged_in()) {
            pb_log_security_event(0, 'outlook_disconnect_ajax_failed', ['reason' => 'not_authenticated']);
        ...
        if ($result) {
            ...
            pb_log_security_event($user_id, 'outlook_disconnect_ajax_failed', ['reason' => 'disconnect_failed']);
            wp_send_json_error('No se pudo desconectar tu cuenta de Outlook. Por favor, intenta de nuevo más tarde.');
        }
    } catch (Exception $e) {
        pb_log_security_event(get_current_user_id(), 'outlook_disconnect_ajax_error', ['error_type' => 'exception', 'message' => $e->getMessage()]);
        ...
        pb_log_security_event(get_current_user_id(), 'outlook_disconnect_ajax_error', ['error_type' => 'fatal', 'message' => $t->getMessage()]);
