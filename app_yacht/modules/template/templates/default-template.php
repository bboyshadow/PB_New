                                        ...
                                        <div class="template-charter-cost-container" style="border-radius: 4px; font-size: 12px; min-width: 300px; background: #f5faf0; padding: 5px;">
                                            ...
                                            <?php if (!empty($resultData['promotionActive']) && !empty($resultData['promotionNights']) && $resultData['promotionNights'] > 0): ?>
                                                <table role="presentation" width="100%" border="0" cellpadding="0" cellspacing="0"
                                                    ...
                                                    <tr>
                                                        <td align="left" valign="middle" style="padding: 5px; color: #4b4f54; width: 70%;">
                                                            Promotion Rate <?php echo htmlspecialchars($resultData['promotionNights']); ?>x<?php echo htmlspecialchars($resultData['nights']); ?> =
                                                        </td>
                                                        <td align="right" valign="middle" style="padding: 5px; text-align: right; color: #4b4f54; width: 30%;">
                                                            <?php echo htmlspecialchars($resultData['promotedRate']); ?>
                                                        </td>
                                                    </tr>
                                                </table>
                                            <?php endif; ?>
                                            ...
                                            <!-- VAT -->
                                            <?php if (empty($templateData['hideElements']['hideVAT']) && (!empty($resultData['vatRateForDisplay']) || !empty($resultData['mixed_breakdown']))): ?>
                                                <?php if (!empty($resultData['mixed_breakdown'])): ?>
                                                    <?php foreach ($resultData['mixed_breakdown'] as $vatItem): ?>
                                                        <table role="presentation" width="100%" border="0" cellpadding="0" cellspacing="0" 
                                                            style="width: 100%; border-spacing: 0; border-collapse: collapse; padding: 2px 5px; color: #4b4f54; font-family: Arial, sans-serif; font-size: 12px;">
                                                            <tr>
                                                                <td align="left" valign="middle" style="padding: 5px; color: #4b4f54; width: 70%;">
                                                                    VAT (<?php echo htmlspecialchars($vatItem['vat_rate']); ?>%): <?php echo htmlspecialchars($vatItem['nights']); ?>N <?php echo htmlspecialchars(substr(strtolower($vatItem['country_name']), 0, 1)); ?>:
                                                                </td>
                                                                <td align="right" valign="middle" style="padding: 5px; text-align: right; color: #4b4f54; width: 30%;">
                                                                    <?php echo htmlspecialchars($vatItem['vat_amount_formatted']); ?>
                                                                </td>
                                                            </tr>
                                                        </table>
                                                    <?php endforeach; ?>
                                                <?php else: ?>
                                                    <?php if (!empty($resultData['vatRateForDisplay']) && floatval($resultData['vatRateForDisplay']) > 0
